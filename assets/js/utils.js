...
function generateShareImage(resultPayload) {
  ...
  ctx.fillStyle = theme.text;
  ctx.font = 'bold 28px sans-serif';
  ctx.fillText('朋友眼中的你', 124, currentY + 42);

  ctx.fillStyle = theme.sub;
  ctx.font = '26px sans-serif';
  drawWrappedText(ctx, resultPayload.friend, 124, currentY + 86, 832, 38);
  ...
  ctx.lineWidth = 2;
  drawRoundedRect(ctx, 94, currentY, 892, 260, 24);
  ctx.fill();
  ctx.stroke();
